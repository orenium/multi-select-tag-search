...
Background worker (`src/background/serviceWorker.js`) is cosmetic only: it `importScripts` the catalog to build `declarativeContent` rules that swap in `icons/icon-active-*.png` (built as `ImageData` via OffscreenCanvas, which MV3 requires), and mirrors the live selection count onto the action badge from `runtime.sendMessage` (`sender.tab.id` avoids needing the `tabs` permission).

Icon rules come from `iconRules()` in the catalog — never hardcode a second list. Each rule is `(domain AND css)`: the `css` half uses each verified site's `chipCss` (a **compound** selector — Chrome rejects combinators `>`, `+`, `~`, descendant spaces and comma lists), so the icon only lights on pages that actually contain chips, not on every page of a supported domain. Chrome evaluates the selectors locally and reports only that a rule fired, so this stays URL-blind. Caveat that drove the `CSS_MATCH_EXEMPT` map: the css matcher counts only **displayed** elements, so a site that hides chips at some viewport widths (GitHub hides repo topics below ~1000px via `hide-sm hide-md`) must fall back to domain-only matching or the icon greys out on narrow windows while the extension still works. Exemptions require a written reason; a test enforces both that and the compound-selector rule.
...
- Verified beyond IMDb (23 total, latest pass 2026-08-19): Letterboxd, Stack Overflow + 4 SE-network sites, GitHub, npm, Steam (numeric tagids parsed from `InitAppTagModal` JSON on app pages), itch.io (path-composed `/games/tag-a/tag-b`), AO3 (base tag path + `other_tag_names` filter), Discogs (`style_exact`/`genre_exact` repeated params), arXiv (advanced-search `cross_list_category` terms), PubMed (`"X"[MeSH Terms] AND ...`), Safebooru + Gelbooru (`tags=a+b`), MyAnimeList (`genre[]=a&genre[]=b`), AniList (GraphQL `genre_in`, cross-checked against the live item-page DOM), Royal Road (`tagsAdd=` repeated param), MusicBrainz (Lucene `tag:"a" AND tag:"b"`), Flickr (`tags=a,b&tag_mode=all`), pixiv (`/tags/a%20b/artworks`).
- A site can fail promotion for reasons beyond "blocked": JustWatch/GitLab/Twitch were tested and confirmed to silently drop the second filter (OR or no-op, not AND) — demoted to `research`, not left as `draft`. Amazon's `rh=` facets ARE a real AND but the facet codes are opaque/per-category with no generic pattern, and the site already offers native checkbox multi-select — demoted to `research` on product-fit grounds, not a verification failure. A `js-rendered` curl result (TMDB, Trakt) only means curl can't check it — it does NOT justify demoting to `research`; leave as `draft` and verify with a real browser tab instead.
