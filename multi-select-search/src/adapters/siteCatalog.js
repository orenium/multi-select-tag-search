...
(function (root, factory) {
  ...
  const SITE_CATALOG = [
    ...
    {
      id: 'myanimelist', label: 'MyAnimeList', category: 'movies-tv', status: 'draft',
      evidence: 'chips /anime/genre/ID/Name confirmed in HTML; anime.php?genres=1,4 returns results but AND unproven (result list caps at 100, no total shown)',
    ...
    {
      id: 'anilist', label: 'AniList', category: 'movies-tv', status: 'draft',
      evidence: 'JS-rendered (curl gets empty shell); known URL /search/anime?genres=A&genres=B unverified',
      host: /(^|\.)anilist\.co$/, path: /^\/(anime|manga)\//,
      tagPattern: /\/search\/(?:anime|manga)\?(?:.*&)?genres=([^&"]+)/,
    ...
    },
    {
      ...
      evidence: 'discover?with_genres=a,b exists but page is JS-rendered; AND vs OR unverified via curl',
    ...
    // ═══ 2. Streaming guides ══════════════════════════════════════════
    { id: 'justwatch', label: 'JustWatch', category: 'streaming', status: 'draft',
      evidence: 'JS-rendered; /us/movies?genres=act,cmy known URL form, semantics unverified',
      host: /(^|\.)justwatch\.com$/, path: /\/(movie|tv-show|tv-series)\//,
      tagPattern: /\/movies\?genres=([a-z0-9,]+)/,
      buildUrl: (vals) => 'https://www.justwatch.com/us/movies?genres=' + vals.join(',') },
    { id: 'trakt', label: 'Trakt', category: 'streaming', status: 'draft',
      evidence: '/movies/popular?genres=a,b accepted; AND vs OR not yet measured (no counts on page)',
    ...
    { id: 'backloggd', label: 'Backloggd', category: 'gaming-community', status: 'draft',
      evidence: '/games/lib/popular?genre=a&genre=b URL accepted; AND unmeasured',
      host: /(^|\.)backloggd\.com$/, path: /^\/games\//,
      tagPattern: /\/games\/lib\/[^?"]*\?(?:.*&)?genre=([a-z0-9-]+)/,
      buildUrl: (vals) => 'https://backloggd.com/games/lib/popular/?' + vals.map(v => 'genre=' + enc(v)).join('&') },
    { id: 'rawg', label: 'RAWG', category: 'gaming-community', status: 'draft',
      evidence: 'API supports tags=a,b (AND per docs); site is JS app, unverified live',
    ...
    { id: 'nexusmods', label: 'Nexus Mods', category: 'gaming-community', status: 'draft',
      evidence: 'mod search supports multiple tag filters in UI; URL form ?tags[]=a&tags[]=b unverified',
    ...
    { id: 'pypi', label: 'PyPI', category: 'code', status: 'draft',
      evidence: 'classifier chips /search/?c=... on project pages; multi-c AND unverified (search blocked to curl by anti-bot challenge)',
      host: /(^|\.)pypi\.org$/, path: /^\/project\//,
      tagPattern: /\/search\/?\?c=([^&"]+)/,
      buildUrl: (vals) => 'https://pypi.org/search/?' + vals.map(v => 'c=' + v).join('&') },
    { id: 'gitlab', label: 'GitLab', category: 'code', status: 'draft',
      evidence: '/explore/projects?topic=a supports one topic officially; multi unverified',
      host: /(^|\.)gitlab\.com$/, path: /^\/[^\/]+\/[^\/]+/,
      tagPattern: /\/explore\/projects\?topic=([^&"]+)/,
      buildUrl: (vals) => 'https://gitlab.com/explore/projects?topic=' + vals.map(enc).join(',') },
    ...
    { id: 'librarything', label: 'LibraryThing', category: 'books', status: 'draft',
      evidence: '/tag/fantasy,magic historically renders a tag-intersection page; not verified this round',
      host: /(^|\.)librarything\.com$/, path: /^\/work\//,
      tagPattern: /\/tag\/([^\/?"]+)$/,
      buildUrl: (vals) => 'https://www.librarything.com/tag/' + vals.map(enc).join(',') },
    { id: 'royalroad', label: 'Royal Road', category: 'books', status: 'draft',
      evidence: 'fictions/search?tagsAdd=a&tagsAdd=b is the site\'s own multi-tag include filter; unverified live',
    ...
    { id: 'scribblehub', label: 'Scribble Hub', category: 'fanfic', status: 'draft',
      evidence: 'series-finder?sf=1&tgi=1,2&mgi=and exposes an explicit AND mode; numeric tag IDs; unverified live',
    ...
    { id: 'rateyourmusic', label: 'Rate Your Music', category: 'music', status: 'draft',
      evidence: 'charts URL /charts/top/album/all-time/g:ambient,dream-pop is the site\'s own multi-genre AND syntax; bot-blocked, unverified',
      host: /(^|\.)rateyourmusic\.com$/, path: /^\/release\//,
      tagPattern: /\/genre\/([^\/?"]+)/,
      buildUrl: (vals) => 'https://rateyourmusic.com/charts/top/album/all-time/g:' + vals.map(enc).join(',') },
    { id: 'bandcamp', label: 'Bandcamp', category: 'music', status: 'draft',
      evidence: 'discover supports one main tag + subfilters; true multi-tag AND unclear',
      host: /(^|\.)bandcamp\.com$/,
      tagPattern: /bandcamp\.com\/tag\/([a-z0-9-]+)/,
      buildUrl: (vals) => 'https://bandcamp.com/discover/' + vals[0] + (vals[1] ? '?tags=' + vals.slice(1).join(',') : '') },
    { id: 'musicbrainz', label: 'MusicBrainz', category: 'music', status: 'draft',
      evidence: 'search supports lucene: tag:a AND tag:b; unverified live',
      host: /(^|\.)musicbrainz\.org$/, path: /^\/(release|release-group|artist)\//,
      tagPattern: /\/tag\/([^\/?"]+)/,
      buildUrl: (vals) => 'https://musicbrainz.org/search?type=release_group&method=advanced&query=' + enc(vals.map(v => 'tag:"' + v + '"').join(' AND ')) },
    { id: 'lastfm', label: 'Last.fm', category: 'music', status: 'research',
      notes: 'tag pages are single-tag; no intersection endpoint' },

    // ═══ 10. Photos ═══════════════════════════════════════════════════
    { id: 'flickr', label: 'Flickr', category: 'photos', status: 'draft',
      evidence: 'search/?tags=a,b&tag_mode=all is Flickr\'s own documented AND mode; JS-rendered, unverified this round',
      host: /(^|\.)flickr\.com$/, path: /^\/photos\/[^\/]+\/\d+/,
      tagPattern: /\/photos\/tags\/([^\/?"]+)/,
      buildUrl: (vals) => 'https://www.flickr.com/search/?tags=' + vals.map(enc).join(',') + '&tag_mode=all' },
    ...
    // ═══ 11. Art & design ═════════════════════════════════════════════
    { id: 'pixiv', label: 'pixiv', category: 'art', status: 'draft',
      evidence: '/tags/A%20B/artworks (space-separated tags) is pixiv\'s native AND search; login-walled for verification',
      host: /(^|\.)pixiv\.net$/, path: /\/artworks\/\d+/,
      tagPattern: /\/tags\/([^\/?"]+)(?:\/artworks)?$/,
      buildUrl: (vals) => 'https://www.pixiv.net/tags/' + vals.map(enc).join('%20') + '/artworks' },
    ...
    { id: 'openalex', label: 'OpenAlex', category: 'academic', status: 'draft',
      evidence: 'works?filter=concepts.id:a,concepts.id:b is documented AND; site UI thin',
    ...
    booruSite('gelbooru', 'gelbooru.com', 'Gelbooru', 'imageboards', 'draft',
      'same Gelbooru engine as verified safebooru.org; not verified directly (mixed-content site)'),
    {
    ...
    { id: 'zerochan', label: 'Zerochan', category: 'imageboards', status: 'draft',
      evidence: '/A,B comma path is the site\'s own multi-tag AND browse; unverified',
      host: /(^|\.)zerochan\.net$/,
      tagPattern: /^\/([^\/?",]+)$/,
    ...
    { id: 'ebay', label: 'eBay', category: 'ecommerce', status: 'draft',
      evidence: 'aspect filters chain in URL path/params and are ANDed per facet; URL shapes are category-specific — needs per-vertical mapping',
    ...
    { id: 'amazon', label: 'Amazon', category: 'ecommerce', status: 'research', notes: 'rh= filter param is cryptic/per-category; not generalizable safely' },
    ...
    { id: 'twitch', label: 'Twitch', category: 'video', status: 'draft',
      evidence: 'directory tag filters compose in URL; tag UUIDs, JS app — needs in-app verification',
      host: /(^|\.)twitch\.tv$/, path: /^\/directory\//,
      tagPattern: /[?&]tl=([A-Za-z0-9-]+)/,
      buildUrl: (vals) => 'https://www.twitch.tv/directory/all?tl=' + vals.join(',') },
    ...
    // ═══ 21. Makers & misc ════════════════════════════════════════════
    { id: 'ravelry', label: 'Ravelry', category: 'makers', status: 'draft',
      evidence: 'pattern search composes attribute facets in URL (pa=...%2B...); mostly behind login now',
      host: /(^|\.)ravelry\.com$/, path: /^\/patterns\//,
      tagPattern: /\/patterns\/search#?.*pa=([^&"]+)/,
      buildUrl: (vals) => 'https://www.ravelry.com/patterns/search#pa=' + vals.map(enc).join('%2B') },
